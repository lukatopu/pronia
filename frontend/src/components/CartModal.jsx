...
function CartModal({ cart = [], isCartClicked, onClose, handleCartOverlay, isCartOverlayHidden }) {
  ...
  return (
   <>
   <div onClick={handleCartLogic} className={`overlay ${isCartOverlayHidden ? '' : 'active'}`}>
      ...
      {cart.length > 0 && (
        <>
          ...
          <div className='buttonsContainer'>
            <Link to="/cart" onClick={handleCartLogic}><button>View Cart</button></Link>
            <Link to="/checkout" onClick={handleCartLogic}><button>Checkout</button></Link>
          </div>
        </>
      )}
    </div>
